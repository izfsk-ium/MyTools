...
func printResult(resultsChan <-chan hostResult, fp *os.File) {
	for e := range resultsChan {
		if len(e.Results) == 0 {
			fmt.Print(".")
			continue
		}
...
func main() {
	...
	go printResult(resultChan, fp_result)

	for i := 1; i < 255; i++ {
		for j := 1; j < 255; j++ {
			go spider(fmt.Sprintf("43.142.%d.%d", i, j), errChan, resultChan)
		}
	}
